...
<html lang="es">
  <head>
    ...
    <title><?php echo $nom_app; ?> | Registro de Pagos de Comprobantes</title>
  ...
  <body class="bg-light" onload="f_SetDimension(); f_Init();" style="zoom: 80%;">
    ...
    <script type="text/javascript">
      function f_Init(){
        ...
        // Titulo de Pantalla
          $("#nv_titulo").html('| Registro de Pagos de Comprobantes');
    ...
    <script type="text/javascript">
      ...
      $(document).ready(function(){
        ...
        $("#pago_entidadbancaria_2").on("change", function(){
          ...
          if (id_banco) {
            // Consultar y cargar cuentas
            $.post("apis/backend.php", { accion: "get_CuentasBancariasPorProveedor", id_banco: id_banco, id_proveedor: id_proveedor, is_gerencia: 1 },
              function(data){
                $combo_cuentas.html('<option value="">Elija una opción...</option>');
    ...
    <script type="text/javascript">
      function f_LoadResultados(){
        ...
        $.post( "apis/backend.php", { accion: "get_ComprobantePago_ListaValorizacion", fecha_inicio_emision: fecha_inicio_emision, fecha_fin_emision: fecha_fin_emision, fecha_inicio_sindetraccion: fecha_inicio_sindetraccion, fecha_fin_sindetraccion: fecha_fin_sindetraccion, fecha_inicio_detraccion: fecha_inicio_detraccion, fecha_fin_detraccion: fecha_fin_detraccion,filtro_lote: filtro_lote, filtro_lote_gel: filtro_lote_gel, is_gerencia: 1 },
          function( data ) {
            if(data.estado == 1){
      ...
      function f_LoadDetallePagos(){
        ...
        $.post("apis/backend.php", { accion: "get_ComprobantePago_RegistroPagos", id_comprobante_pago: id_recepcion },
          function(data){
            if(data.estado == 1){
      ...
      function f_GetTipoCambio(){
        ...
        // Cargando datos
          ...
          $.post("apis/backend.php", { accion: "get_TipoCambio", fecha },
            function(data){
              if(data.estado == 1){
                data.data.forEach(d => {
    ...
    <script>
      ...
      function fmtUSD(n){ return '$ ' + (new Intl.NumberFormat('en-US', {minimumFractionDigits:2, maximumFractionDigits:2})).format(n || 0); }
      function fmtPEN(n){ return 'S/ ' + (new Intl.NumberFormat('es-PE', {minimumFractionDigits:2, maximumFractionDigits:2})).format(n || 0); }
      function pct(paid, total){ return (total > 0) ? Math.max(0, Math.min(100, (paid/total)*100)) : 0; }
      ...
      $('#modal_registraradelanto').on('shown.bs.modal', function(){ f_RenderResumenPagos(); });
    ...
    <script type="text/javascript">
      function f_GrabarComprobante(){
        ...
        // Grabando datos
          ...
          $.post("apis/backend.php", { accion: "grabar_ComprobantePago_Valorizacion", modograbar_comprobante: modo, id_proveedor, id_valorizacion_detalle: vals_str, id_moneda: id_moneda, comprobante_serie: serie, comprobante_numero: numero, comprobante_fecha: fecha, sub_total: total, comprobante_porc_detraccion: porc_detraccion, tipo_cambio },
            function(data){
              if (data.estado == 1){
                f_LoadResultados();
      ...
      function f_GrabarTipoCambio(){
        ...
        $.post('apis/backend.php', { accion: 'grabar_config_tipocambio', modo: modo, id: id, fecha: fecha, compra: compra, venta: venta, id_moneda_base: id_moneda_base, },
          function(response){
