...
<head>
	...
	<title><?php echo $nom_app; ?> | Cumplimiento - Transportista</title>
...
<section>
	<div class="container-fluid" id="content">
						...
						<div class="row" style="border: solid; border-width: 1px; border-color: #E6E9ED; border-radius: 7px; background-color: #ffffff; margin-bottom: 5px;">
													...
													if ($res_transportistas = mysqli_query($enlace, $q_transportistas)){
														if (mysqli_num_rows($res_transportistas) > 0) {
															while($row_transportistas = mysqli_fetch_array($res_transportistas)){
																?>
																<option value="<?php echo $row_transportistas["Id"]; ?>"><?php echo $row_transportistas["razon_social"]; ?></option>
																<?php
															}
														}
													}
							...
							<div class="row" style="border: solid; border-width: 1px; border-color: #E6E9ED; border-radius: 7px;">
								...
								<div class="col-md-12 col-sm-12 col-xs-12" style="padding: 20px; margin-top: -15px; overflow-x: scroll; width: 100%;">
									...
									<input id="hd_id_origendato" type="hidden" value="2">
		...
		<script type="text/javascript">
			...
			// Titulo de Pantalla
				$("#nv_titulo").html('| Cumplimiento - Transportista');
		...
		<script type="text/javascript">
			...
			function f_LoadResultados(){
				...
				$.post( "apis/backend.php", { accion: "get_ListaDetalleControlInternoCliente", filtro_tipocliente: filtro_tipocliente, filtro_cliente: filtro_cliente, id_origendato: id_origendato }, 
					function( data ) {
						if(data.estado == 1){
							...
							$.each( data.res, function( key, val ) {
								...
								_html += ' <button class="btn btn-success " type="button"  style="font-size: 14px; padding: 5px; width: 30px; margin-bottom: 5px" onclick="f_ReporteGestionDocumentaria('+val.Id+", '"+val.documento+"','"+val.razon_social+"',"+val.cod_tipocliente+')">'
								_html += '   <i class="bi bi-file-earmark-text"></i>  '
								_html += ' </button>'

								_html += ' <button class="btn btn-warning " type="button" title="Reporte Vertical" style="font-size: 14px; padding: 5px; width: 30px; margin-bottom: 5px" onclick="f_ReporteGestionDocumentariaVertical('+val.Id+", '"+val.documento+"','"+val.razon_social+"',"+val.cod_tipocliente+')">'
								...
								if(arr_proceso_detalle_tipo_dato.length>0){
									for (var i = 0; i < arr_proceso_detalle_tipo_dato.length; i+=1) {
										...
										var bloquear_celda = id_tipo_cliente != '' && id_tipo_cliente != val.cod_tipocliente ? "lightgray" : "none";

										if(id_tipo_dato==99999999){
											var ocultar_td = tipo_dato_detalle_mas[2] != val.cod_tipocliente ? "lightgray" : "";
							...
							$.each( data.res, function( key, val ) {
								...
								if(is_excel == 0){
	     						...
	     						//Tipo cliente 1: Natural / 2: Jurídico
									if(id_tipo_cliente != val.cod_tipo_cliente){
										 ...
										 $('#input_id_tipo_dato_'+val.Id+'_'+id_cliente).daterangepicker({
										 	...
										 	if(val.input_value != ''){
												...
												if(diferencia_dias >= 0){
													...
													html_label_vigencia = "<font class='bg bg-danger vencimiento otro_tipo_font_"+id_cliente+"_"+val.id_detalle+"'  style='color: white' ><i class='bi bi-exclamation-circle-fill'></i> Venció hace "+ ((diferencia_dias)*-1) +" día(s)</font>";
												}
										  ...
										  $('#input_id_tipo_dato_'+val.Id+'_'+id_cliente).on('apply.daterangepicker', function(ev, picker) {
									        ...
									        var fecha_picker = picker.startDate.format('DD/MM/YYYY') + ' - ' + picker.endDate.format('DD/MM/YYYY');
										      ...
										      //Formatear Fecha de Vigencia FIN
													...
													if(diferencia_dias >= 0){
														html_label_vigencia = "<font class='bg bg-warning vencimiento otro_tipo_font_"+id_cliente+"_"+val.id_detalle+"' id='font_id_tipo_dato_"+val.Id+"_"+id_cliente+"'><i class='bi bi-exclamation-circle-fill'></i> Vence en "+ diferencia_dias +" día(s)</font>";
													}else{
														html_label_vigencia = "<font class='bg bg-danger vencimiento otro_tipo_font_"+id_cliente+"_"+val.id_detalle+"' id='font_id_tipo_dato_"+val.Id+"_"+id_cliente+"' style='color: white' ><i class='bi bi-exclamation-circle-fill'></i> Venció en "+ ((diferencia_dias)*-1) +" día(s)</font>";
													}
