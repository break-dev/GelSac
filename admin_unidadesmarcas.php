...
<html lang="es">
	<head>
  	...
  	<link rel="stylesheet" href="<?php echo $url_lims?>/global/styles.css">
		...
		<title><?php echo $nom_app; ?> | Administración de Marcas de Unidades</title>
	...
	<body class="bg-light" onload="f_SetDimension(); f_Init();" style="zoom: 80%;">
	          ...
	          <div class="modal-content">
				...
				<div class="col-md-12 col-sm-12 col-xs-12" style="border: solid; border-width: 1px; border-color: #E6E9ED; border-radius: 7px; padding-top: 10px; padding-left: 35px;">
					<div class="d-flex row">
						<div class="row text-end" style="border: solid; border-width: 1px; border-color: #E6E9ED; border-radius: 7px; background-color: #ffffff; margin-bottom: 5px; ">
						...
						<div class="row" style="border: solid; border-width: 1px; border-color: #E6E9ED; border-radius: 7px; background-color: #ffffff; ">
							<div class="row" style="padding: 20px;">
								<div class="col-md-9 col-sm-9 col-xs-9">
									<div class="d-flex">
										<h5>Resumen de Marcas de Unidades</h5>
								...
								<div class="col-md-3 col-sm-3 col-xs-3" style="margin-top: -5px;">
									<button class="btn btn-primary" type="button" onclick="f_AdminMarca('x');" style="color: #ffffff; width: 100%; font-size: 14px;">
			              <b> + Nueva Marca</b>
			            </button>
								</div>
		...
		<!-- Ventanas modales -->
		<div class="modal fade" id="modal_addmarca" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modal_addmarcaLabel" aria-hidden="true">
		  <div class="modal-dialog">
		    <div class="modal-content">
		      <div class="modal-header">
		        <h1 class="modal-title fs-6" id="modal_addmarcaLabel"></h1>
		        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
		      </div>
		      ...
		      <input id="hd_idmarca" type="hidden">
		      <input id="hd_modograbar" type="hidden">

		      <div class="modal-footer">
		        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
		        <button type="button" class="btn btn-primary" onclick="f_GrabarMarca();">Grabar</button>
		      </div>
		    </div>
		  </div>
		</div>
		...
		<script type="text/javascript">
			function f_Init(){
				...
				// Titulo de Pantalla
					$("#nv_titulo").html('| Administración de Marcas de Unidades');
        ...
        $.post( "apis/backend.php", { accion: "get_ListaMarcasUnidades", filtro_descripcion: filtro_descripcion }, 
          function( data ) {
            if(data.estado == 1){
              $.each( data.res, function( key, val ) {
                ...
                // Agregando acciones
                  ...
                  _html += '      <a class="success" href="javascript: f_AdminMarca(' + d + ', ' + val.Id + ", '" + val.descripcion	+ "'" + ')"><i class="bi bi-pencil-square"></i>';
    	...
    	function f_AdminMarca(_item, _id_modalidad, _descripcion){
    		// Definiendo título de ventana e Inicilizando controles de tipo texto
          if (_item != 'x'){
            tipo = "E";
            titulo = 'Editar Marca: "<b>' + _descripcion + '</b>"';
	        }
	        else{
            tipo = "N";
            titulo = "Nueva Marca";
	        ...
	        $("#modal_addmarcaLabel").html(titulo);

		    // Identificando el tipo de grabación
	        $("#hd_modograbar").val(tipo);

		    // Cargando datos
	        f_OpenModal('modal_addmarca');

	        if (tipo != 'N'){
            $("#hd_idmarca").val(_id_modalidad);
		...
		<script type="text/javascript">
			// Graba información temporal (onblur).
				function f_GrabarMarca(){
					// Recupera variables
						var id_registro = $("#hd_idmarca").val();
          ...
          // Grabando Datos
            $.post( "apis/backend.php", { accion: "grabar_MarcasUnidades", modo_grabar: modo_grabar, id_registro: id_registro, descripcion: modalidad_descripcion },
              function( data ) {
                if (data.estado == 2){
                  alert("La Marca ingresada ya fue registrada anteriormente.\nPor favor verificar.");
                  ...
                  return;
	                ...
	                if(data.estado == 1){
	                	...
	                	f_cerrarModal('modal_addmarca');
	                }
	                else{
	                  alert("Ocurrió un error al momento de grabar la Marca.");
	                }
	              }
        ...
        function f_CambiarEstado(_Estado, _id_registro){
          ...
          if(confirm("¿Está seguro de " + estado + " la Marca seleccionada?")){
            $.post( "apis/backend.php", { accion: "update_MarcasUnidades", id_registro: _id_registro, estado: _Estado }, 
              function( data ) {
                if(data.estado == 1){
                  f_LoadResultados();
                }
      ...
      // Eliminar registros
        function f_EliminarRegistro(_id_registro){
          if(confirm("¿Está seguro de eliminar la Marca seleccionada?\n\nSi continua perderá la información permanentemente. ¿Desea continuar?")){
            $.post( "apis/backend.php", { accion: "eliminar_MarcasUnidades", id_registro: _id_registro },
              function( data ) {
                if(data.estado == 1){
                  f_LoadResultados();
                }
