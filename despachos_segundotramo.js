document.addEventListener("DOMContentLoaded", function () {
    ...
    function formatNumber(num, decimals = 2) {
        ...
        }).format(num).replace(/,/g, ''); // Remove commas for input fields
    }
    ...
    window.f_LoadDistribuciones = function () {
        ...
        $('#div_unidades_planta').html('<div class="text-center p-3 text-muted"><div class="spinner-border spinner-border-sm" role="status"></div> Cargando...</div>');
            ...
            if (response.estado === 1) {
                ...
                if (response.data && response.data.length > 0) {
                    response.data.forEach(function (row) {
                        ...
                        // safely encode JSON for data-obj
                        const obj_json = btoa(unescape(encodeURIComponent(JSON.stringify(row))));
                        const placas_display = `<b>${row.placa1}</b>${row.placa2 ? ' / ' + row.placa2 : ''}`;
                        const fecha_display = row.fecha_hora_llegada ? row.fecha_hora_llegada : row.fecha_estimada;

                        html += `
                        <div class='card mb-2 shadow-sm ${border_color} pointer' style='cursor: pointer;' onclick='f_AbrirPesaje(${row.id_distribucion})' id='tr_dist_${row.id_distribucion}' data-obj='${obj_json}'>
                ...
                $('#div_unidades_planta').html(`<div class='alert alert-danger text-center small p-3'>${response.mensaje || 'Error al consultar la BD.'}</div>`);
            }
        }).fail(function () {
            $('#wt_loadingunidades').hide();
            $('#div_unidades_planta').html('<div class="alert alert-danger text-center small p-2">Error al conectar con el servidor.</div>');
        });
    }
    ...
    window.f_AbrirPesaje = function (id_distribucion) {
        ...
        $('#div_unidades_planta .card').removeClass('border-primary shadow');
        element.addClass('border-primary shadow');
        ...
        $('#lbl_placas').html(data.placa1 + (data.placa2 ? ' / ' + data.placa2 : ''));
        ...
        f_callBackend('get_detalle_distribucion_to_despacho', {
            ...
            let html_detalles = '';
            if (response.estado === 1 && response.data && response.data.length > 0) {
                ...
                detalles.forEach((det, index) => {
                    ...
                    if (parseFloat(neto) > 0 || parseFloat(tara) > 0 || parseFloat(bruto) > 0) {
                        ...
                        <div class="card-body bg-light py-3">
                            <div class="row g-3 align-items-center">
                                <div class="col-md-3">
                                    ...
                                    <input type="number" ${attr_disabled} class="form-control text-center fw-bold fs-6 py-2 shadow-sm border-dark" id="txt_tara_${det.id_distribucion_detalle}" value="${tara}" onkeyup="f_CalcularNetoLote(${det.id_distribucion_detalle})">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold text-muted mb-1"><i class="bi bi-truck-front-fill text-secondary"></i> Bruto (Kg)</label>
                                    <input type="number" ${attr_disabled} class="form-control text-center fw-bold fs-6 py-2 shadow-sm border-dark" id="txt_bruto_${det.id_distribucion_detalle}" value="${bruto}" onkeyup="f_CalcularNetoLote(${det.id_distribucion_detalle})">
                                </div>
                                <div class="col-md-2" style="${showBigBags}">
                                    ...
                                    <input type="number" ${attr_disabled} class="form-control text-center fw-bold fs-6 shadow-sm py-2 border-primary text-primary bg-info bg-opacity-10" id="txt_bbs_${det.id_distribucion_detalle}" value="${bigbags}">
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label small fw-bold text-success mb-1"><i class="bi bi-box"></i> Neto (Kg)</label>
                                    <input type="text" class="form-control text-center fw-bold fs-6 py-2 bg-white text-dark shadow-sm border-success" id="txt_neto_${det.id_distribucion_detalle}" data-seco="${det.peso_seco}" data-tomado="${det.peso_tomado}" value="${formatNumber(neto)}" readonly>
                                </div>
                                <div class="col-md-2 text-end mt-4">
                                    <button style="margin-top: 15px !important" class="btn ${is_locked ? "btn-secondary" : "btn-primary"} w-100 fw-bold shadow-sm py-2" style="border-radius: 8px;" id="btn_save_${det.id_distribucion_detalle}" onclick="f_GuardarPesajeLote(${det.id_distribucion_detalle}, ${tipo_carga})" ${attr_disabled}>
                                        <i class="bi ${is_locked ? "bi-check2-all" : "bi-save"} me-1"></i> ${is_locked ? "Guardado" : "Guardar"}
                                    </button>
                                </div>
            ...
            if (response.data && response.data.length > 0) {
                setTimeout(() => {
                    response.data.forEach((det) => {
                        if (parseFloat(det.peso_neto) > 0 || parseFloat(det.peso_tara) > 0 || parseFloat(det.peso_bruto) > 0) {
            ...
            $('#div_detalles_lotes').html(`<div class='alert alert-danger text-center small py-2'>Error al obtener detalles.</div>`);
        });
    ...
    window.f_CapturarPesoLote = function (tipo, id_detalle) {
        if (!$("#chk_auto_pesaje").prop('checked')) {
            alert("El pesaje automático está desactivado. Escriba el peso manualmente.");
            return;
        }
    ...
    window.f_CalcularNetoLote = function (id_detalle) {
        ...
        if (tara > 0 && bruto > 0) {
            ...
            $(`#txt_neto_${id_detalle}`).removeClass('text-danger text-decoration-line-through');

            // Logica de alertas
            const peso_seco = parseFloat($(`#txt_neto_${id_detalle}`).attr('data-seco')) || 0;
            const peso_tomado = parseFloat($(`#txt_neto_${id_detalle}`).attr('data-tomado')) || 0;
            let warning_msg = "";
    ...
    window.f_GuardarPesajeLote = function (id_detalle, tipo_carga) {
        ...
        f_callBackend('update_PesajesSegundoTramo', payload).done(function (response) {
            btn.html(originalText).prop('disabled', false);
            if (response.estado === 1) {
                // Visual confirmation
                btn.removeClass('btn-success').addClass('btn-outline-success');
                setTimeout(() => { btn.removeClass('btn-outline-success').addClass('btn-success'); }, 2000);
            ...
            if (response.estado === 1) {
                ...
                if (response.data && response.data.length > 0) {
                    response.data.forEach(function (row) {
                        ...
                        let active_class = (active_id == row.id_distribucion) ? 'border-primary shadow' : border_color;
