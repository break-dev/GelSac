<?php
  ...
  // Ruta imágenes
    ...
    $ruta_images = substr($ruta_images_x, 0, strpos($ruta_images_x, 'print_primertramo_guiat_v1.php')) . 'images/';
    $ruta_images_qr = substr($ruta_images_x, 0, strpos($ruta_images_x, 'print_primertramo_guiat_v1.php')) . '/';
  ...
  // 1. Obteniendo datos de la guía
    ...
    $q_datos = "SELECT DISTINCT
                 ...
                 WHERE MD5(V.guiatransportista_serie) = '".$serie_guia."'
                   AND MD5(V.guiatransportista_numero) = '".$numero_guia."'
      ...
      if (mysqli_num_rows($res_datos) > 0) {
        while($row_datos = mysqli_fetch_array($res_datos)){
          ...
          // Genera en línea el código QR
            $url = 'https://gelerp.intelli-apps.com/print_primertramo_guiat_v1.php?a='.$serie_guia.'&b='.$numero_guia.'&c='.$id_remitente.'&d='.$id_transportista.'&e='.$guia_fecha;
            $dir = 'images/qr/';
  ...
  // 3. Arma la estructura de Detalle
    ...
    $q_datos = "SELECT VD.lote_cod_lote,
                  ...
                  FROM despachos_primertramo_validaciondatos VD
                       ...
                       INNER JOIN tb_clientes ET ON T.id_Transportista = ET.Id
                 WHERE MD5(VD.guiatransportista_serie) = '".$serie_guia."'
                   AND MD5(VD.guiatransportista_numero) = '".$numero_guia."'
