<?php
	...
	// 1. Obteniendo datos de la guía
		...
		$q_datos = "SELECT DISTINCT
                       ...
                       UPPER(ET.direccion) AS TRANSPORTISTA_DIRECCION,
      ...
      if (mysqli_num_rows($res_datos) > 0) {
        while($row_datos = mysqli_fetch_array($res_datos)){
          ...
          $transportista_direccion = ((strlen(trim($row_datos["TRANSPORTISTA_DIRECCION"])) == 0) ? '---' : $row_datos["TRANSPORTISTA_DIRECCION"]);
	...
	// 2. Arma la estructura de Detalle
		...
		$q_datos = "SELECT VD.lote_cod_lote,
								  ...
								  FROM despachos_primertramo_validaciondatos VD
								  		 ...
								  		 INNER JOIN tb_clientes ET ON T.id_Transportista = ET.Id
                 WHERE MD5(guiatransportista_serie) = '".$serie_guia."'
                   AND MD5(guiatransportista_numero) = '".$numero_guia."'
      ...
      if (mysqli_num_rows($res_datos) > 0) {
        ...
        // Completa con líneas adicionales
        	...
        	while ($a < 8){
	        	...
	        	if ($a == 4){
	        		$adicional_descripcion = 'Según G.R.R.: <label style="font-family: AgencyFBb">'.$guiaR.'</label>';
	        	}
	        	else{
		        	if ($a == 5){
